...
同样的，分割成独立的$P(\bar{p_n})$项也是很有用的。比如，我们可能想把每个光线发出的量的项分成多个小光源发出的光的量。$L_{e,s}$和从大的光源拆分的量L_{e,l},可得下式:
...
这两部分积分可以被独立开来计算，有可能两部分用的是完全不同的算法，或者不同数量的样本，或者根据不同场景选择处理方式等。也包括$L_{e,s}$的估计，忽略了任何从巨大的光源的发光亮，对L_{e,l}的估计忽略了小光源的发光量，并且所有光都被大或小进行了分类。最终计算出正确的结果。
...
2. 给定特定一项$P(\bar{p}_i)$,为了计算多条路径的积分的蒙特卡洛估计值，如何生成一条或多条路径$\bar{p}$?
...
给定一组顶点$p_0,p_1,...p_{i-1}$,按这样的方式采样，那么我们采样的最后一个点$p_i$就在光源上，以同样的方式来定义它的PDF。虽然我们能使用与采样路径顶点相同的方式来采样光源上的点，但是这样经常会导致较高的差异，因此对于所有$p_i$不在光源表面上的路径，路径的贡献就会为0。期望值还是此积分的正确值，但是收敛会变得极其慢。更好的方式是，只在发光物体的表面上采样，并且这些表面的概率已经被相应更新。给定一个完整的路径，我们就有足够的信息来计算$P(\bar{p}_i)$的估计值，此计算只需要计算每一项的值即可。
