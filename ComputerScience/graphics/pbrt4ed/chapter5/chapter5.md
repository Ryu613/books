...
给出了入射辐射量的函数，我们就能定义胶片上某点的入射光的辐照度。先从公式4.7开始，用辐射度来得到辐照度的定义，然后，我们可以利用公式4.9，把在立体角上的积分转换为在面积上的积分(在这种情况下，平面上某个区域$A_e$，与镜头的背面相切)。这样就可以得到在胶片平面上的点p的辐照度公式:
...
> $\theta'$是p'和法线的夹角
>
> 之所以取$\vert \cos \theta \cos \theta'\vert$,是对$L_i(p,p')$做了两次朗伯余弦，代表了从透镜到p',从p'到p的夹角导致的光辐射衰减
>
> $\vert |p'-p||^2$代表从点p'到p的距离，除以此项是用来表示距离与光的强度呈平方反比衰减
...
把这些细节先搁置一旁，现在我们会假设某些滤波器函数$f$是被用来定义感光器在图像像素周围的响应上随空间的变化量。这些滤波器函数很快就会趋于0，这样做是模拟现实里胶片上的像素点只对接近这个点的光做出响应。同时，这些滤波器也模拟了像素点响应的空间上的其他变化量。用这种方式，若我们有一个图像函数$r(x,y)$，这个函数在胶片上的任意一点给出了红色(比如利用公式5.6中，利用感光器响应函数$\bar{r}(\lambda)$来测量的值)，然后，滤波后在点(x,y)的红色值$r_f$就被下式给出:
