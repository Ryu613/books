...
把这些细节先搁置一旁，现在我们会假设某些滤波函数$f$是被用来定义感光器在图像像素周围的响应上随空间的变化量。这些滤波函数很快就会趋于0，这样做是模拟现实里胶片上的像素点只对接近这个点的光做出响应。同时，这些滤波器也模拟了像素点响应的空间上的其他变化量。用这种方式，若我们有一个图像函数$r(x,y)$，这个函数在胶片上的任意一点给出了红色(比如利用方程5.6中，利用感光器响应函数$\bar{r}(\lambda)$来测量的值)，然后，滤波后在点(x,y)的红色值$r_f$就被下式给出:
...
为被积函数采样，用了2种方法，首先，均匀的在图像上采样，每个图像样本会对多个像素点得最终值产生贡献量，取决于所使用的滤波函数的范围。这种方法的估计如下式:
