...
傅里叶分析的其中一个基础是傅里叶变换，傅里叶变换是用频域表示的（一般是以空间域来表达)。考虑在图8.2中的两个函数图像，在8.2(a)的函数随着x的变化相对缓慢，然而在8.2(b)的函数相对变化更快。变化更慢的函数被称为具有更低的频率内容

> 频率内容：多个不同频率下振幅组成的整个分布，类似于spectrum(频谱)
>
> 高频代表变化块，低频代表变化慢
