...
. CMAKE_BUILD_TYPE： is a STRING type
.. Debug(default)
.. Release
.. MinSizeRel
.. RelWithDebInfo
...
. generator expression: omitted
. target_precompile_headers() : compile a pch header for target to reduce compile time
.. REUSE_FROM can reuse config of pch to other target
